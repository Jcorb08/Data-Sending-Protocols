...
public class Receiver extends NetworkHost {
    ...
    private boolean check(Packet p){
        ...
        for (int i = 0; i < p.getPayload().length(); i++) {
            sum += (int) p.getPayload().charAt(i);
        }
        sum += p.getAcknum() + p.getChecksum() + p.getSeqnum();    
        // add all data and checksum together should be -1 or 1111111 etc. for binary
        return sum == -1;
    }
    ...
    private int getChecksum(int seq, int ack){
        
        // ~ here used for ones complement
        return Integer.parseUnsignedInt(Integer.toBinaryString(~(seq + ack)), 2);
    }    
    ...
    public void input(Packet packet) {
        // each possibility for that state
        boolean corrupted = !(check(packet));
        boolean inSeq = (packet.getSeqnum() == seqNum);
        if(!corrupted && inSeq){
            deliverData(packet.getPayload());
            // 0 is a positive ACK
            Packet sndACKpkt = new Packet(packet.getSeqnum(), 0, getChecksum(packet.getSeqnum(), 0));
            udtSend(sndACKpkt);
            seqNum += 1;
            if (seqNum > 1){
                seqNum = 0;
        ...
        else if(!corrupted && !inSeq){
            // 0 is a positive ACK
            Packet sndACKpkt = new Packet(packet.getSeqnum(), 0, getChecksum(packet.getSeqnum(),0));
            udtSend(sndACKpkt);
        }
        else if(corrupted){
            // 1 is a negative ACK or NAK
            Packet sndNAKpkt = new Packet(packet.getSeqnum(),1,getChecksum(packet.getSeqnum(), 1));
            udtSend(sndNAKpkt);
        }
